...
async function abrirModalMensagem(idSolicitacao) {
    try {
        solicitacaoAtualId = Number(idSolicitacao);

        if (!Number.isInteger(solicitacaoAtualId)) {
            mostrarFeedback('danger', 'ID da solicitação não encontrado.');
            return;
        }

        const modalElement = document.getElementById('mensagensModal');
        if (!modalElement) {
            mostrarFeedback('danger', 'Modal de mensagens não encontrado.');
            return;
        }
        ...
        console.log('abrindo modal para solicitacaoAtualId:', solicitacaoAtualId);

        await marcarMensagensComoLidas(solicitacaoAtualId);
        await carregarMensagensModal(solicitacaoAtualId);

    } catch (error) {
        console.error('Erro ao abrir modal de mensagens:', error);
        mostrarFeedback('danger', 'Não foi possível abrir as mensagens.');
    }
}

async function carregarMensagensModal(idSolicitacao) {
    try {
        const response = await fetch(`${API_URL}/api/mensagens/${idSolicitacao}`, {
            method: 'GET',
            headers: {
        ...
        if (!response.ok) {
            ...
            container.innerHTML = `<p class="text-danger">${obterMensagemErro(data, 'Não foi possível carregar as mensagens.')}</p>`;
            return;
        }
        ...
        if (!mensagens.length) {
            container.innerHTML = `<p class="text-muted">Nenhuma mensagem ainda.</p>`;
            return;
        }

        container.innerHTML = mensagens.map(msg => {
            const isMinha = msg.id_remetente === usuarioLogado.id;

            const label = isMinha
                ? (usuarioLogado.role === 'admin' ? 'Admin' : 'Cliente')
                : (usuarioLogado.role === 'admin' ? 'Cliente' : 'Admin');
            return `
        <div class="mb-2 p-2 rounded ${isMinha ? 'bg-primary text-white text-end' : 'bg-light'}">
            <strong>${label}</strong><br>
            <div>${msg.conteudo}</div>
            <small>
                ${new Date(msg.enviada_em).toLocaleString()}
                ${isMinha ? (msg.lida ? '✓ Lida' : '✓ Enviada') : ''}
            </small>
        </div>
    `;
        }).join('');
        ...
        mostrarFeedback('danger', 'Erro de conexão ao carregar as mensagens.');
    }
}
...
async function enviarMensagemModal() {
    try {
        ...
        if (!mensagem) {
            mostrarFeedback('warning', 'Digite uma mensagem para enviar.');
            return;
        }
        ...
        const response = await fetch(`${API_URL}/api/mensagens/${solicitacaoAtualId}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
        ...
        if (response.ok) {
            input.value = '';
            mostrarFeedback('success', 'Mensagem enviada com sucesso!');
            await carregarMensagensModal(solicitacaoAtualId);
        } else {
            const data = await response.json();
            mostrarFeedback('danger', obterMensagemErro(data, 'Não foi possível enviar a mensagem.'));
        }
    } catch (error) {
        console.error('Erro ao enviar mensagem:', error);
        mostrarFeedback('danger', 'Erro de conexão ao enviar a mensagem.');
    } finally {
        esconderLoading();
    }
}

async function marcarMensagensComoLidas(idSolicitacao) {
    try {
        await fetch(`${API_URL}/api/mensagens/${idSolicitacao}`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
...
function abrirModalMensagens(event) {
    const botaoMensagens = event.target.closest('.btn-mensagens');
    if (!botaoMensagens) return;

    const idSolicitacao = botaoMensagens.dataset.id;
    abrirModalMensagem(idSolicitacao);
}
                ...
                timeStyle: 'medium'
                    ...
                    <td>
                        <div class="d-flex flex-column gap-2 align-items-end> ">
                            ...
                            <button type="button" class="btn btn-secondary btn-sm btn-mensagens" data-id="${solicitacao.id}">
                                Mensagens
                                <span id="badgeMensagens" class="badge bg-danger" style="display:none;">0</span>
                            </button>
                        </div>
                    </td>
                </tr>
...
async function atualizarBadgeMensagens() {
    try {
        const response = await fetch(`${API_URL}/api/mensagens/nao-lidas`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
...
setInterval(atualizarBadgeMensagens, 10000);
...
if (tabelaSolicitacaoAdmin) {
    ...
    tabelaSolicitacaoAdmin.addEventListener('click', abrirModalMensagens);
}

const confirmarMensagemBtn = document.getElementById('confirmarMensagemBtn');
if (confirmarMensagemBtn) {
    confirmarMensagemBtn.addEventListener('click', enviarMensagemModal);
}
