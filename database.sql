...
CREATE TABLE mensagens (
    ...
    CONSTRAINT fk_mensagem_solicitacao
        ...
        REFERENCES solicitacaoEmprestimo(id)
        ON DELETE CASCADE,
    ...
    CONSTRAINT chk_conteudo_nao_vazio
        CHECK (length(trim(conteudo)) > 0)
);

CREATE INDEX idx_mensagens_solicitacao
    ON mensagens (id_solicitacao);

CREATE INDEX idx_mensagens_destinatario_lida
    ON mensagens (id_destinatario, lida);

CREATE INDEX idx_mensagens_solicitacao_data
    ON mensagens (id_solicitacao, enviada_em);